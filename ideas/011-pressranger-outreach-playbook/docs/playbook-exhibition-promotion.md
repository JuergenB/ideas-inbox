...
A repeatable SOP so every open call and every exhibition gets promoted the **same way, every time** — instead of depending on whoever has time. Built from the Thursday Open Call meeting. The engine is **PolyWiz** (URL → AI content → approval → schedule → publish; "Open Call" is a scaffolded campaign type; paid ads = [polywiz-app#181](https://github.com/JuergenB/polywiz-app/issues/181)); **Artwork Archive's Airtable** holds the open-call records, submissions, and the contact/listing registry that feed it; **PressRanger is one plugged-in feed** (journalist data + pitch radar). This document is the *process*; the build lives in the PolyWiz epic. See [channel-mix-and-budget.md](../research/channel-mix-and-budget.md) for the grounded architecture.
...
| **Paid-ads engine** | PolyWiz ([Idea 007](../../007-polywiz-paid-ads-engine/)) | Juergen | New **"Open Calls" content category**: auto-generates 8–15 ad variants → Meta + Pinterest via Zernio, optimized for *submissions*, plus organic reminder posts across the call's run. Conservative **~$150–$300 per-call flight**. Full mix + budget: [channel-mix-and-budget.md](../research/channel-mix-and-budget.md). |
