...
- **Standing owner clarification (2026-08-12):** when the owner commissions an idea expressly *to share it* with a named outside person or group ("share this with our fundraising expert", "the ideas archive I can share"), that instruction IS the creation-time public decision — scaffold it directly HERE, not in `ideas-studio`. In the owner's usage, `ideas-studio` is **internal-only**; "the ideas archive" for shareable material means this repo. Consequence: every file must be written as outward-shareable from the first commit (no commercial detail, no internal operational notes, no private-repo links). Recorded after a 2026-08-12 session nearly scaffolded a deck-for-an-external-expert in studio.
